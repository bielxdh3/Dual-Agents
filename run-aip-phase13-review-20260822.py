...
config = replace(
    ...
    runs_dir=root / "runs-review-phase13",
    require_clean_git=False,
)
agent = replace(config.agent_for_role("reviewer"), reasoning_effort="low")
output = root / "aip-phase13-review-20260822.result.json"
result = run_codex_app_server(
    config=config,
    agent=agent,
    repository=repository,
    prompt=(root / "aip-phase13-review-20260822.md").read_text(encoding="utf-8"),
    output_path=output,
    session_id="biel4-review-phase13-f39f6d2",
    request_id="aip-phase13-review-final-20260822",
    run_id="aip-phase13-review-final-20260822",
    role="reviewer",
)
