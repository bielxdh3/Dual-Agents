...
_AUTH_STATUS_LABELS = {
    ...
    "CLI_SETUP_REQUIRED": "CLI setup required",
    "DISABLED": "Disabled",
}
...
class DashboardService:
    ...
    def _auth_actions(account: Any) -> list[str]:
        if account.backend == "claude_code":
            return ["status", "authenticate", "reauthenticate"]
    ...
    def create_profile(self, body: dict[str, Any]) -> dict[str, Any]:
        ...
        allowed = {
            ...
            "fallback_roles", "provider_type", "adapter_type", "auth_mode",
        }
        ...
        for field in ("codex_home", "model", "reasoning_effort", "base_url", "auth_reference", "runtime_model", "fixed_mode", "provider_type", "adapter_type", "auth_mode"):
    ...
    def _start_auth(self, name: str, *, reauthenticate: bool) -> dict[str, Any]:
        ...
        if account.backend == "claude_code":
            return {
                ...
                "actions": self._auth_actions(account),
                "message": "Run the official Claude Code login flow interactively (claude, then /login); Dual Agents never handles Claude credentials.",
            }
    ...
    def save_settings(self, name: str, body: dict[str, Any]) -> dict[str, Any]:
        ...
        allowed = {
            ...
            "base_url", "auth_reference", "auth_mode", "available_models", "supported_reasoning_efforts",
        }
        unknown = sorted(set(body) - allowed)
        if unknown:
        ...
        if auth_mode is not None and auth_mode not in {"provider_native", "environment", "none"}:
            raise DashboardError("auth_mode must be one of provider_native, environment, none.")
        if auth_reference:
            try:
        ...
        if selected is not None and target_backend == "antigravity":
            ...
            runtime_model = ""
            fixed_mode = ""
        if selected is not None and target_backend == "claude_code" and not advertised_efforts:
            # Selecting a no-effort Claude model must clear any effort saved
            # for the previously selected model, even when the browser omits
            # the disabled selector from the PATCH body.
            if effort is not None and chosen_effort:
        ...
        if target_backend == "claude_code":
            if chosen_tier:
                raise DashboardError("Claude Code does not advertise service tiers.")
            chosen_tier = ""
        ...
        updated = update_account_settings(
            ...
            service_tier=chosen_tier if (tier is not None or target_backend == "claude_code") else None,
            backend=backend,
