...
config = replace(
    ...
    runs_dir=root / "runs-review-phase9-final",
    require_clean_git=False,
)
agent = replace(config.agent_for_role("reviewer"), reasoning_effort="low")
output = root / "aip-phase9-final-review-final-result-20260820.json"
result = run_codex_app_server(
    config=config,
    agent=agent,
    repository=Path(r"E:\AIP"),
    prompt=(root / "aip-phase9-final-review-final-20260820.md").read_text(encoding="utf-8"),
    output_path=output,
    session_id="biel4-review-phase9-final-dfeee2f",
    request_id="aip-phase9-final-review-final-20260820",
    run_id="aip-phase9-final-review-final-20260820",
    role="reviewer",
)
